...
YC_DP_FOLDER_ID = dataproc_cluster_info["YC_DP_FOLDER_ID"]  # '<идентификатор_каталога>'
YC_DP_SSH_PUBLIC_KEY = dataproc_cluster_info[
    "YC_DP_SSH_PUBLIC_KEY"
]  # '<открытая_часть_SSH-ключа>'
YC_DP_SUBNET_ID = dataproc_cluster_info["YC_DP_SUBNET_ID"]  # '<идентификатор_подсети>'
YC_DP_GROUP_ID = dataproc_cluster_info[
    "YC_DP_GROUP_ID"
]  # '<идентификатор_группы_безопасности>'
YC_DP_SA_ID = dataproc_cluster_info[
    "YC_DP_SA_ID"
]  # '<идентификатор_сервисного_аккаунта>'
YC_DP_METASTORE_URI = dataproc_cluster_info[
    "YC_DP_METASTORE_URI"
]  # '<IP-адрес>' thrift://10.40.0.16:9083
YC_DP_AZ = "ru-central1-a"
YC_SOURCE_BUCKET = dataproc_cluster_info["YC_SOURCE_BUCKET"]  # 'yc-dataproc21-tasks'
YC_DATA_BUCKET = dataproc_cluster_info["YC_DATA_BUCKET"]  # 'yc-dataproc21-data'
...
dag = DAG(
    dag_id="demographic_datamart_dag",
...
with dag:
    ...
    create_spark_cluster = DataprocCreateClusterOperator(
        ...
        properties={  # С указанием на удалённый кластер Metastore.
            ...
            "spark:spark.jars": f"s3a://{YC_SOURCE_BUCKET}/jars/delta-core_2.12-2.3.0.jar,s3a://{YC_SOURCE_BUCKET}/jars/delta-storage-2.3.0.jar,s3a://{YC_SOURCE_BUCKET}/jars/clickhouse-jdbc-0.6.5.jar",
    ...
    # 2 этап: запуск задания PySpark — инициализация данных витрины в S3
    init_datamart = DataprocCreatePysparkJobOperator(
        task_id="pyspark_init_datamart",
        main_python_file_uri=f"s3a://{YC_SOURCE_BUCKET}/data/7.4-datamart-init.py",
        connection_id=ycSA_connection.conn_id,
        dag=dag,
    )

    # 3 этап: запуск задания PySpark — загрузка данных из витрины на S3 в витрину на ClickHouse
    datamart_clickhouse_upload = DataprocCreatePysparkJobOperator(
        task_id="datamart_clickhouse_upload",
        main_python_file_uri=f"s3a://{YC_SOURCE_BUCKET}/data/7.4-datamart-clickhouse.py",
        connection_id=ycSA_connection.conn_id,
        dag=dag,
    )

    # 4 этап: удаление кластера Yandex Data Processing
    delete_spark_cluster = DataprocDeleteClusterOperator(
        task_id="dp-cluster-delete-task", trigger_rule=TriggerRule.ALL_DONE, dag=dag
    )

    # Формирование последовательности выполнения из указанных выше этапов
    (
        create_spark_cluster
        >> init_datamart
        >> datamart_clickhouse_upload
